...
class SemCUTAblationModel(BaseModel):
    ...
    def get_ablation_paths(self):
        return self.ablation_path
    ...
    def data_dependent_initialize(self, data):
        ...
        self.real_A_rand = self.real_A_rand[:bs_per_gpu]
...
#
    ...
    def set_input(self, input):
        ...
        self.real_A_rand = input['A_rand'].to(self.device)
        ...
        self.ablation_path = input['ablation_img_path']
        
    def manipulate(self, x):
        return x
    
    def encode_real(self, real):
        real_latent, mres_enc = self.netenc(
            self.manipulate(real), 
    ...
    def forward_style(self):
        real_latent_seg, mres_enc_seg = self.encode_real(self.real_A_rand)
        real_latent_style, mres_enc_style = self.encode_real(self.real_A)
        if self.use_seg_decoder:
            with torch.no_grad():
                _, mres_mask = self.decode_seg(real_latent_seg, mres_enc_seg)
                #mres_mask = [torch.zeros_like(el) for el in mres_mask]
        else:
            mres_mask = None

        self.fake = self.decode_style(real_latent_style, mres_mask)
